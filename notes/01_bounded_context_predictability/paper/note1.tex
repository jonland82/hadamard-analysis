...
The result resolves the first fork in the larger narrative.  ``Globally choreographed'' is exact: orthogonality, normalization, and balance are mathematical constraints.  ``Locally hard to predict'' is true after arbitrary equivalent presentations are admitted: the permuted signal collapses to the balance baseline.  ``Locally informative'' is also true, but conditionally: the catalog coordinate convention exposes a transferable signal.  The final phrase---``enough to guide an exact search''---remains a question, not a result.
...
This proposition supplies the bridge to the next experimental note: train on catalog-aligned coordinates, use the score only to choose which sign to try first, and measure first-solution nodes and backtracks against exact baselines.  A poor predictor may slow the search, but it cannot make the search incorrect.
...
So the signal is not nowhere; it is somewhere specific.  It lives in the representative.  That makes the next question both narrower and sharper: can an exact search choose the same language and exploit the signal without ever mistaking a preference for a proof?
