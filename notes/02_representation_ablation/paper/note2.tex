...
\takeaway{A solver can calculate the score from its current exact state, and the score strongly prefers the continuation found in held-out valid matrices.  This is an offline bridge from global structure to a local preference; whether that preference survives its own search trajectory is a separate empirical question.}
...
Most importantly, the observed block is only one continuation from a known complete matrix.  A highly ranked counterfactual block may pass \eqref{eq:feasible} yet admit no full completion.  Offline percentile does not equal first-solution speedup.  The follow-up solver study embeds $L_{\mathrm{all}}$ in a complete search, holds exact pruning fixed, and compares nodes and backtracks with random, lexicographic, balance-only, and minimum-pressure orderings.  Statistical scores reorder feasible branches; they do not delete one.

That online test sharpens the limitation.  The all-pair score usually beats weak structural policies, but its advantage reverses against a lexicographic baseline as the hidden region grows.  In the full order-$12$ construction from two seed rows, lexicographic search has a median of $74$ visited nodes, whereas the hybrid regional policy has a median of $1{,}385$.  The exact laws here remain correct and the offline ranking remains strong; what fails to compose is the inference from plausible local shape to a globally completable branch.
...
Thus global choreography does point locally, but its pointing device is not the last few symbols.  It is the evolving constraint state.  The subsequent exact-solver experiment shows why the distinction matters: knowing the likely composition of the next region does not identify which locally plausible region extends to a complete matrix.  Regional prediction is a real structural result, but completability---or a reliable proxy for branch survival---is the next algorithmic target.
